import { DATA_VERSION, lineName, lineOrders, lines, stations } from './data.js';
...
export function buildGraph() {
  ...
  transferPairs.forEach(([stationId, leftLine, rightLine]) => {
    const left = nodeId(leftLine, stationId);
    const right = nodeId(rightLine, stationId);
    const transfer = (from, to) => edgeBase('transfer', from, to, {
      stationId,
      cost: 1,
      transferStatus: 'pending-verification',
      notes: 'Transfer movement is explicitly modelled; duration, platform path, and accessibility are not verified in this demo.',
    });
...
function routeCost(edges, objective, accessibility) {
  const transfers = edges.filter((edge) => edge.type === 'transfer').length;
  const rides = edges.filter((edge) => edge.type === 'ride').length;
  const unknownAccess = edges.filter((edge) => edge.accessibility === 'unknown').length;
  const accessViolations = accessibility?.stepFree && unknownAccess ? 1 : 0;
  const walking = transfers;
  const signature = routeSignature(edges);
  if (objective === 'fewest-changes') return [transfers, rides, walking, accessViolations, signature];
  if (objective === 'accessible') return [accessViolations, transfers, rides, walking, signature];
  return [rides + transfers, transfers, walking, accessViolations, signature];
}

function search(graph, starts, destinations, objective, accessibility, blockedEdgeIds = new Set(), penalties = new Set()) {
  const queue = starts.map((start) => ({ node: start, edges: [], cost: routeCost([], objective, accessibility) }));
  const best = new Map();
  ...
  while (queue.length) {
    ...
    outgoing.forEach((edge) => {
      ...
      const cost = routeCost(edges, objective, accessibility);
      cost[0] += penalty;
...
function buildSteps(edges) {
  ...
  edges.forEach((edge, index) => {
    ...
    if (edge.type === 'transfer') {
      steps.push({ type: 'change', stationId: from.stationId, fromLine: lineName(from.lineId), toLine: lineName(to.lineId), edgeId: edge.id, status: edge.status, text: `Change from ${lineName(from.lineId)} to ${lineName(to.lineId)} at ${stations[from.stationId].name}.` });
      previousLine = null;
      return;
    }
    if (previousLine !== from.lineId) {
      steps.push({ type: 'ride', stationId: from.stationId, lineId: from.lineId, lineName: lineName(from.lineId), direction: edge.direction, edgeId: edge.id, status: edge.status, text: `Take ${lineName(from.lineId)} ${edge.direction === 'toward-end' ? 'toward the terminal' : 'toward the start'}.` });
...
export function planJourney({ originStationId, destinationStationId, objective = 'fastest', accessibility = {}, blockedEdgeIds = [] }) {
  ...
  const found = search(graph, starts, destinations, objective, accessibility, new Set(blockedEdgeIds));
  if (!found) return { status: 'unreachable', message: 'No route is available in this static network snapshot.' };

  const primary = makeResult(found.edges, objective, accessibility, originStationId, destinationStationId);
  const alternatives = [];
  const seen = new Set([routeSignature(found.edges)]);
  found.edges.forEach((edge) => {
    const candidate = search(graph, starts, destinations, objective, accessibility, new Set(blockedEdgeIds), new Set([edge.id]));
    if (candidate && !seen.has(routeSignature(candidate.edges))) {
      seen.add(routeSignature(candidate.edges));
      alternatives.push(makeResult(candidate.edges, objective, accessibility, originStationId, destinationStationId));
    }
  });
  return { status: 'success', route: primary, alternatives: alternatives.slice(0, 2), networkVersion: DATA_VERSION };
}

function makeResult(edges, objective, accessibility, originStationId, destinationStationId) {
  const transfers = edges.filter((edge) => edge.type === 'transfer').length;
  ...
  const route = {
    ...
    confidence: accessibility?.stepFree ? 'limited — accessibility data is unverified' : 'limited — static topology only',
    freshness: 'verified-static topology; operational data unavailable',
    explanation: objective === 'fewest-changes'
      ? 'Ordered by fewest explicit line changes, then ride segments. Transfer movement is not timed in this demo.'
      : objective === 'accessible'
        ? 'Accessibility preference applied, but station equipment and transfer paths are not verified in this demo.'
        : 'Scheduled travel time is unavailable; ordered by the best available static topology proxy.',
  };
  return route;
}
